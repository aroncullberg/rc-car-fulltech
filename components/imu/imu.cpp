...
void sensor::IMU::updateFromConfig() {
...
esp_err_t sensor::IMU::configureIMU() {
...
esp_err_t sensor::IMU::setFullScaleRanges() {
...
bool sensor::IMU::validDeviceId() {
    return icm20948_check_id(&icm_device_) != ICM_20948_STAT_OK;
}

esp_err_t sensor::IMU::initializeDMP() {
    bool success = true;
    ...
    icm_20948_DMP_data_t dummy_data;
    for (int i = 0; i < 5; i++) {
        inv_icm20948_read_dmp_data(&icm_device_, &dummy_data);
        vTaskDelay(pdMS_TO_TICKS(10));
    }
...
void sensor::IMU::imuTask(void *parameters) {
        ...
        if ((status == ICM_20948_STAT_OK) || (status == ICM_20948_STAT_FIFO_MORE_DATA_AVAIL)) {
            ...
            if (dmp_data.header & DMP_header_bitmap_Accel) {
                ...
                if (instance->log_accel_) {
                    ...
                    ESP_LOGI(TAG, "Accel: x=%+5.3f g, y=%+5.3f g, z=%+5.3f g",
...
int16_t applyDeadband(const int16_t value, const int16_t bias, const int16_t deadband) {
    const auto diff = static_cast<int16_t>(value - bias);
    return std::abs(diff) < deadband ? static_cast<int16_t>(0) : diff;
}
